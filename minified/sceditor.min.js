/* SCEditor v3.2.0 | (C) 2017, Sam Clarke | sceditor.com/license */ ! function() {
    ...
    var bt = {
            toolbar: "undo,redo|bold,italic,underline,strike,subscript,superscript|left,center,right,justify|font,size,color,removeformat|cut,copy,pastetext|bulletlist,orderedlist,indent,outdent|table|toc|code,quote|horizontalrule,image,email,link,unlink|emoticon|ltr,rtl|maximize,source",
            toolbarExclude: null,
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var targetElement = document.querySelector('.sceditor-button-horizontalrule');

        // Insert the select element before the target element
        targetElement.parentNode.insertBefore(selectElement, targetElement);
        ...
        selectElement.addEventListener("change", function() {
            ...
            if (selectedValue) {
                alert('Selected heading level: ' + selectedValue);
            }
        });
    ...
    var Pe = {
            ...
            horizontalrule: {
                _dropDown: function(editor, button, callback) {
                    ...
                    // Add options for each heading level
                    for (var i = 1; i <= 6; i++) {
                    ...
                    dropdown.addEventListener("change", function() {
                        ...
                        // Close the dropdown after selecting the heading level
                        editor.closeDropDown(true);
                    });
            
                    // Create the dropdown below the button
                    editor.createDropDown(button, "heading-dropdown", dropdownContainer);
                },
                exec: function(button) {
                    var editor = this;
                    var selectedText = editor.getRangeHelper().selectedHtml();
            
                    if (selectedText) {
                        // Check if the selected text is already wrapped in a heading tag
                        var existingHeadingMatch = selectedText.match(/^<h[1-6](?:\s+[a-zA-Z]+=".*?")*>/i);

                        console.log(existingHeadingMatch)

                        var existingHeading = existingHeadingMatch ? existingHeadingMatch[0] : null;
            
                        // Remove any existing heading tag wrapping the selected text
                        if (existingHeading) {
                            selectedText = selectedText.replace(existingHeading, '').replace(new RegExp('</' + existingHeading.slice(1), 'i'), '');
                        }
            
                        // Remove surrounding block-level elements
                        selectedText = selectedText.replace(/<\/?(?:div|p)[^>]*>/gi, '');
            
                        editor.commands.horizontalrule._dropDown(editor, button, function(selectedHeading) {
                            if (selectedHeading) {
                                // Insert the selected text wrapped in the chosen heading tag
                                editor.wysiwygEditorInsertHtml('<' + selectedHeading + '>' + selectedText + '</' + selectedHeading + '>');
                            }
                        });
                    } else {
                        alert('Please select the text you want to change the heading of.');
                    }
                },
                tooltip: "Change Heading Level"
            },
            code: {
                exec: function() {
