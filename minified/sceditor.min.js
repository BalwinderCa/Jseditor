/* SCEditor v3.2.0 | (C) 2017, Sam Clarke | sceditor.com/license */ ! function() {
    "use strict";
    ...
    function ht(e) {
        var t, n, o, r, i, a, l = Re(e, "whiteSpace"),
            c = /line$/i.test(l),
            s = e.firstChild;
    ...
    var bt = {
            toolbar: "undo,redo|bold,italic,underline,strike,subscript,superscript|left,center,right,justify|font,size,color,removeformat,lineHeight|cut,copy,pastetext|bulletlist,orderedlist,indent,outdent|table|toc|code,quote|image,email,link,unlink|emoticon|ltr,rtl|maximize,source",
            toolbarExclude: null,
    ...
    function generateTableOfContents() {
        // Get the iframe containing the SCEditor content
        const iframe = document.querySelector('iframe'); // Adjust the selector as needed
      
        // Access the document inside the iframe
        const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
      
        // Get all headings (h1, h2, etc.) from the iframe content
        const headings = Array.from(iframeDocument.querySelectorAll('h1, h2, h3, h4, h5, h6'));
      
        // Generate the table of contents HTML
        let tocHtml = '';
        if (headings.length > 0) {
            tocHtml += `<p style="min-height:0.001em;"></p><div id="this_area_is_uneditable" style="border: 1px solid #dedede;background-color: #fafafa;padding: 20px;color: #cebbeb;"><ul style="list-style-type: none;">`;
            headings.forEach((heading) => {
                ...
                // Define a class based on the heading level
                let headingClass = '';
                switch (headingLevel) {
                    case 'h1':
                        headingClass = 'h1-class'; // Change this to your desired class for h1 headings
                        break;
                    case 'h2':
                        headingClass = 'h2-class'; // Change this to your desired class for h2 headings
                        break;
                    case 'h3':
                        headingClass = 'h3-class'; // Change this to your desired class for h3 headings
                        break;
                    case 'h4':
                        headingClass = 'h4-class'; // Change this to your desired class for h4 headings
                        break;
                    case 'h5':
                        headingClass = 'h5-class'; // Change this to your desired class for h5 headings
                        break;
                    case 'h6':
                        headingClass = 'h6-class'; // Change this to your desired class for h6 headings
                        break;
                    // Add more cases for other heading levels if needed
                }
                tocHtml += `<li class="${headingClass}" style="font-size: 14px;font-weight: bold;margin-bottom: 5px;"><a href="#${headingId}" style="color: #a87fe5;text-decoration: none;">${headingText}</a></li>`;
            });
            tocHtml += '</ul></div>';
        }
    ...
    // Event delegation for dynamically generated table of contents links
    document.addEventListener('click', function(event) {
        if (event.target.tagName === 'A' && event.target.parentNode.tagName === 'LI') {
            event.preventDefault();
            const targetId = event.target.getAttribute('href').substring(1);
            smoothScroll(targetId, 800); // Adjust the duration as needed
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        anchorElement.className = 'sceditor-button sceditor-button-image has-icon';
        anchorElement.style.width = '82px';
        anchorElement.style.backgroundColor = 'white';

        // Create the select element
        var selectElement = document.createElement('select');
        selectElement.id = 'line-height-select';
        selectElement.style.border = 'none';
        selectElement.style.outline = 'none';

        // Add option for paragraph (no heading)
        var paragraphOption = document.createElement("option");
        paragraphOption.value = 'p';
        paragraphOption.textContent = 'Paragraph';
        selectElement.appendChild(paragraphOption);

        // Add options for heading levels 1 through 6
        for (var i = 1; i <= 6; i++) {
            var optionElement = document.createElement('option');
            optionElement.value = 'h' + i;
            optionElement.textContent = 'Heading ' + i;
            selectElement.appendChild(optionElement);

            // Set font size based on heading level
            if (i === 1) {
                optionElement.style.fontSize = '24px'; // h1
            } else if (i === 2) {
                optionElement.style.fontSize = '20px'; // h2
            } else if (i === 3) {
                optionElement.style.fontSize = '18px'; // h3
            } else if (i === 4) {
                optionElement.style.fontSize = '16px'; // h4
            } else if (i === 5) {
                optionElement.style.fontSize = '14px'; // h5
            } else if (i === 6) {
                optionElement.style.fontSize = '12px'; // h6
            }
        }

        // Set the default selected value to h1
        selectElement.value = 'h1';
        // Append the select element to the anchor element
        anchorElement.appendChild(selectElement);
        // Find the element with the specified class
        var targetElement = document.querySelector('.sceditor-button-image');

        // Insert the anchor element before the target element
        targetElement.parentNode.insertBefore(anchorElement, targetElement);

        // Add event listener for the button click
        selectElement.addEventListener("change", function() {
            // Execute the callback with the selected heading level
            var selectedValue = selectElement.value;
            var selectedOption = selectElement.selectedOptions[0];
            var attrValue = selectedOption.getAttribute('textarea_selected_id');
            if (selectedValue) {
                replaceTagInIframe(selectedValue,attrValue)
            }
        });
        
        
        document.querySelector('iframe').contentWindow.document.body.addEventListener('click', function(event) {

            // Get the closest parent HTML tag of the clicked element
            var parentTag = event.target.closest('*').tagName.toLowerCase();

            var parentText = event.target.closest('*').textContent.trim();
        
            // Get the select element by its ID
            var selectElement = document.getElementById('line-height-select');

            var uniqueId = generateUniqueId(35);
        
            // Check the value of the parent tag and set the selected option in the select element accordingly
            switch (parentTag) {
                case 'h1':
                    selectElement.value = 'h1';
                    break;
                case 'h2':
                    selectElement.value = 'h2';
                    break;
                case 'h3':
                    selectElement.value = 'h3';
                    break;
                case 'h4':
                    selectElement.value = 'h4';
                    break;
                case 'h5':
                    selectElement.value = 'h5';
                    break;
                case 'h6':
                    selectElement.value = 'h6';
                    break;
                case 'p':
                ...
                case 'div':
                    ...
                    break;
                default:
                    // If the parent tag is not one of the expected values, do nothing
                    break;
            }
            ...
            var options = selectElement.querySelectorAll('option[textarea_selected_id]');
            options.forEach(function(option) {
                option.removeAttribute('textarea_selected_id');
            });
            var options = selectElement.querySelectorAll('option');
            options.forEach(function(option) {
                option.setAttribute('textarea_selected_id', uniqueId);
            });
            
        });

        document.querySelector('iframe').contentWindow.document.body.addEventListener('input', function(event) {
            var target = event.target;
            if (target.classList.contains('image-alt-text')) {
                var img = target.closest('.tooltip').querySelector('img');
                if (img) {
        ...
        function setupEventListener() {
            ...
            iframeDocument.body.addEventListener('click', function(event) {
                ...
                if (target.tagName.toLowerCase() === 'img') {
                    if (!isShown) {
                        target.style.border = '2px solid #3779eb';
                        const buttons = iframeDocument.querySelectorAll('.align-buttons button');
                        buttons.forEach(function(button) {
                            button.style.visibility = 'visible';
                            button.contentEditable = 'false';
                        });
                        const tooltiptext = iframeDocument.querySelectorAll('.tooltiptext');
                        tooltiptext.forEach(function(button) {
                            button.style.visibility = 'visible';
                            button.contentEditable = 'false';
                ...
                if (target.tagName.toLowerCase() === 'div' || target.tagName.toLowerCase() === 'p') {
                    ...
                    const button = iframeDocument.querySelectorAll('.tooltiptext');
        ...
        if (iframeDocument.readyState === 'complete') {
            ...
            // Wait for the load event if the iframe content is not yet loaded
            iframe.addEventListener('load', setupEventListener);
        }


        function replaceTagInIframe(tagName, uniqueId) {
            var iframe = document.querySelector('iframe');
            var iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
            var elements = iframeDocument.querySelectorAll('div[id="' + uniqueId + '"], p[id="' + uniqueId + '"], h1[id="' + uniqueId + '"], h2[id="' + uniqueId + '"], h3[id="' + uniqueId + '"], h4[id="' + uniqueId + '"], h5[id="' + uniqueId + '"], h6[id="' + uniqueId + '"]');
            elements.forEach(function(element) {
                var newElement = document.createElement(tagName);
                newElement.innerHTML = element.innerHTML;
                newElement.setAttribute('id', uniqueId); // Add the id attribute
        ...
        // Define a function to count words and characters in the editor content
        function countWordsAndCharacters() {
            // Get the editor content
            var content = document.querySelector('iframe').contentWindow.document.body.innerText;
            ...
            return { words: words.length, characters: characters };
        }
    
        // Update the word and character count display
        function updateWordAndCharacterCountDisplay() {
            // Get the word and character counts
            var counts = countWordsAndCharacters();
    
            // Update the display element
            var countDisplay = document.getElementById('word-count-display');
            if (countDisplay) {
                countDisplay.textContent = `Words: ${counts.words}, Characters: ${counts.characters}`;
            }
        }
    
        // Call the updateWordAndCharacterCountDisplay function whenever the editor content changes
        document.querySelector('iframe').contentWindow.document.addEventListener('input', updateWordAndCharacterCountDisplay);
    
        // Initialize the word and character count display
        updateWordAndCharacterCountDisplay();
    
        // Find all elements with the class "sceditor-group"
        const sceditorGroups = document.querySelectorAll('.sceditor-group');
    
        // Get the last sceditor-group element
        const lastGroup = sceditorGroups[sceditorGroups.length - 1];
    
        // Define the span element with the desired ID and initial text
        const spanElement = document.createElement('div');
        spanElement.id = 'word-count-display';
        spanElement.style ="vertical-align: text-top;padding: 10px;text-align: end;";
        spanElement.textContent = 'Words: 0, Characters: 0';
    
        // Append the span element after the last sceditor-group element
        if (lastGroup) {
            lastGroup.parentNode.insertBefore(spanElement, lastGroup.nextSibling);
        }
        ...
        document.querySelector('iframe').contentWindow.document.body.addEventListener('click', function(event) {
            ...
            if (target.classList.contains('image_align_left') ||
                target.classList.contains('image_align_center') ||
                target.classList.contains('image_align_right')) {
                
                // Find the container of the clicked button
                const container = target.closest('.align-buttons');
                ...
                alignClosestDiv(target, target.classList[0].split('_')[2]);
            }
        });
        
        function alignClosestDiv(button, alignment) {
            const closestDiv = button.closest('.tooltip').parentNode;
        
            if (closestDiv) {
                closestDiv.style.textAlign = alignment;
            }
        }
    });

    // Function to find the closest ancestor div, p, h1-h6 element containing the given node
    function findContainingElement(node) {
        while (node) {
            if (node.nodeType === Node.ELEMENT_NODE && ['div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(node.tagName.toLowerCase())) {
                return node;
            }
    ...
    var v = {
        html: '<!DOCTYPE html><html{attrs}><head><meta http-equiv="Content-Type" content="text/html;charset={charset}" /><link rel="stylesheet" type="text/css" href="{style}" /></head><body contenteditable="false" {spellcheck}><p></p></body></html>',
        ...
        sizeOpt: '<a class="sceditor-fontsize-option" data-size="{size}" href="#"><font size="{size}">{value}</font></a>',
        ...
        image: '<div><label>{upload}</label></div><div><input type="file" id="fileInput" accept="image/*" /></div><div><input type="button" class="button" value="{insert}" /></div>',
    ...
    var Pe = {
            ...
            lineHeight: {
                _dropDown: function(editor, button, callback) {
                    var dropdownContainer = document.createElement("div");
                    dropdownContainer.style.display = "flex";
                    dropdownContainer.style.flexDirection = "column";
                    dropdownContainer.style.flexWrap = "nowrap";
                    dropdownContainer.style.justifyContent = "center";
                    dropdownContainer.style.lineHeight = "24px";
                    dropdownContainer.style.fontWeight = "600";
                    dropdownContainer.style.cursor = "pointer";
                    
                    // Add event listener for click on dropdown options
                    dropdownContainer.addEventListener("click", function(event) {
                        if (event.target.tagName === "A") {
                            var lineHeightValue = event.target.getAttribute("data-line-height");
                            callback(lineHeightValue);
                    ...
                    // Create options for each line height value
                    for (var value in lineHeightValues) {
                        var option = document.createElement("a");
                        option.setAttribute("data-line-height", value);
                        option.textContent = lineHeightValues[value];
                        dropdownContainer.appendChild(option);
                    }
                    
                    // Create the dropdown below the button
                    editor.createDropDown(button, "line-height-picker", dropdownContainer);
                },
                exec: function(button) {
                    var editor = this;
                    
                    // Call the dropdown function to display options
                    editor.commands.lineHeight._dropDown(editor, button, function(selectedLineHeight) {
                        // Execute the callback with the selected line height value

                        var iframe = document.querySelector('iframe'); // Adjust selector as needed
                        ...
                        if (selection && selection.rangeCount > 0) {
                            ...
                            var commonAncestorNode = range.commonAncestorContainer;
                
                            // Find the closest ancestor div element containing the selection
                            var containingDiv = findContainingElement(commonAncestorNode);
                
                            // Apply line height to the containing div
                            if (containingDiv) {
                                containingDiv.style.lineHeight = selectedLineHeight;
                            }
                        }
                    });
                },
            ...
            pastetext: {
                exec: function(button) {
                    var textToPaste = navigator.clipboard.readText().then(function(clipboardText) {
                        // Insert the clipboard text directly into the editor
                        this.wysiwygEditorInsertText(clipboardText);
                    }.bind(this));
            
                    // Close the dropdown after pasting the text
            ...
            toc: {
                exec: function() {
                    ...
                    // Remove existing table of contents if it exists
                    const existingTOC = iframeDocument.getElementById('this_area_is_uneditable');
                    if (existingTOC) {
                        existingTOC.remove();
                    }

                    const existingPTag = iframeDocument.querySelector('p[style="min-height:0.001em;"]');
                    if (existingPTag) {
                        existingPTag.remove(); // Remove the existing <p> tag
                    }
                        
                    // Generate and insert the new table of contents HTML
                    const tocHtml = generateTableOfContents();
                    this.wysiwygEditorInsertHtml(tocHtml);
            
                    // Attempt to access the iframe's document directly after a brief delay to ensure the content is loaded
                    setTimeout(() => {
                        // Find the div with id 'this_area_is_uneditable' and set contenteditable="false"
                        const tocDiv = iframeDocument.getElementById('this_area_is_uneditable');
                        if (tocDiv) {
            ...
            image: {
                exec: function (caller) {
                    ...
                    var self = this; // Store reference to 'this' for use inside event listener
                    ...
                    fileInput.addEventListener('change', function() {
                        ...
                        if (file) {
                            ...
                            reader.onload = function (event) {
                                var img = new Image();
                                img.onload = function() {
                                    // Calculate the width proportional to the editor's width
                                    var maxWidth = self.getContentAreaContainer().offsetWidth;
                                    var width = img.width > maxWidth ? maxWidth : img.width;
            
                                    var pictureHtml = '<div class="tooltip">';
                                    pictureHtml += '<picture>';
                                    pictureHtml += '<source srcset="' + event.target.result + '" type="' + file.type + '" />';
                                    pictureHtml += '<img style="cursor:pointer;" src="' + event.target.result + '" width="' + width + '" />';
                                    pictureHtml += '</picture>';
                                    pictureHtml += '<div class="tooltiptext">';
                                    pictureHtml += '<div class="align-buttons">';
                                    pictureHtml += '<button class="image_align_left">Left</button>';
                                    pictureHtml += '<button class="image_align_center">Center</button>';
                                    pictureHtml += '<button class="image_align_right">Right</button>';
                                    pictureHtml += '</div>';
                                    pictureHtml += '<input type="text" style="width: 94%;padding: 3px;margin-top: 5px;" placeholder="Enter image alt text" class="image-alt-text">';
                                    pictureHtml += '</div>';
                                    pictureHtml += '</div>';
                                    
                                    self.wysiwygEditorInsertHtml(pictureHtml);
                                };
            ...
            email: {
                _dropDown: function(editor, caller, callback) {
                    ...
                    content.querySelector(".button").addEventListener("click", function(e) {
                        var email = content.querySelector("#email").value;
                        var description = content.querySelector("#des").value;
            
                        if (email) {
                            callback(email, description);
                        }
                ...
                exec: function(caller) {
                    ...
                    // Get the selected text within the iframe
                    var selection = iframeDoc.getSelection();
                    var selectedHtml = selection.toString().trim();
            
                    // Regular expression to match email address and extract the full email
                    var emailRegex = /^(?:mailto:)?([^\s@]+@[^\s@]+)/;
            
                    // Match the selected HTML against the regex
                    var match = selectedHtml.match(emailRegex);
            
                    if (match) {
                        // Extract the full email
                        var email = match[1]; // Get the matched email address
            
                        // Create the dropdown content with the prepopulated email
                        var content = document.createElement("div");
                        content.innerHTML = `
                            <label for="email">E-mail:</label>
                        ...
                        content.querySelector(".button").addEventListener("click", function(e) {
                            var updatedEmail = content.querySelector("#email").value;
                            var updatedDescription = content.querySelector("#des").value;
            
                            if (updatedEmail) {
                                var emailLink = match[0].startsWith("mailto:") ? updatedEmail : `mailto:${updatedEmail}`;
            
                                // Get the range of the selection
                                var range = selection.getRangeAt(0);
            
                                // Find the nearest ancestor anchor element
                                var parentAnchor = range.commonAncestorContainer;
                                while (parentAnchor && parentAnchor.nodeName !== "A") {
                                    parentAnchor = parentAnchor.parentNode;
                                }
            
                                // Replace the parent anchor tag with the new anchor element
                                if (parentAnchor && parentAnchor.nodeName === "A") {
                                    var newAnchor = document.createElement("a");
                                    newAnchor.setAttribute("href", emailLink);
                                    newAnchor.textContent = ze(updatedDescription || updatedEmail);
                                    parentAnchor.parentNode.replaceChild(newAnchor, parentAnchor);
                                } else {
                                    // If no anchor tag is found, insert the anchor element at the cursor position
                                    editor.wysiwygEditorInsertHtml(`<a href="${emailLink}">${ze(updatedDescription || updatedEmail)}</a>`);
                                }
                            }
                            setTimeout(() => {
                                ...
                                if (iframe) {
                                    ...
                                    // Iterate through each anchor tag and set contenteditable="true"
                                    for (let i = 0; i < anchorTags.length; i++) {
                                        anchorTags[i].setAttribute('contenteditable', 'false');
                        ...
                        // If the selected text doesn't contain an email, proceed as usual
                        // Create the dropdown with empty fields
                        var content = document.createElement("div");
                        content.innerHTML = `
                            <label for="email">E-mail:</label>
                            <input type="text" id="email" />
                        ...
                        content.querySelector(".button").addEventListener("click", function(e) {
                            ...
                            if (email) {
                                editor.wysiwygEditorInsertHtml(`<a href="mailto:${ze(email)}">${ze(description || email)}</a>`);
                            }
                            setTimeout(() => {
                                // Find the iframe by its ID
                                const iframe = document.querySelector('iframe');;
                ...
                tooltip: "Insert/Update an email"
            },                        
            link: {
                _dropDown: function(editor, caller, callback) {
                    var content = document.createElement("div");
                    ...
                    content.querySelector(".button").addEventListener("click", function(e) {
                        var url = content.querySelector("#url").value;
                        var description = content.querySelector("#des").value;
                ...
                exec: function(caller) {
                    ...
                    if (selection.rangeCount > 0) {
                        ...
                        while (node) {
                            if (node.nodeName === 'A') {
                                selectedHtml = node.outerHTML.trim();
                                break;
                            }
                            node = node.parentNode;
                        }
                    ...
                    // Regular expression to match URL and extract the full URL
                    var urlRegex = /(?:<a[^>]*href=["']([^"']*)["'][^>]*>)([^<]*)<\/a>/i;
            
                    // Match the selected HTML against the regex
                    var match = selectedHtml.match(urlRegex);
                    ...
                    if (match) {
                        ...
                        // Create the dropdown content with the prepopulated URL and description
                        var content = document.createElement("div");
                        content.innerHTML = `
                            <label for="url">URL:</label>
                            <input type="text" id="url" value="${url}" />
                            <label for="des">Description (optional):</label>
                            <input type="text" id="des" value="${description}" />
                            <input type="button" class="button" value="Update" />
                        `;
            
                        content.querySelector(".button").addEventListener("click", function(e) {
                            var updatedUrl = content.querySelector("#url").value;
                            var updatedDescription = content.querySelector("#des").value;
                        
                            if (updatedUrl) {
                                // Create a new anchor tag with the updated URL and description
                                var newAnchor = `<a href="${ze(updatedUrl)}">${ze(updatedDescription || updatedUrl)}</a>`;
                                ...
                                if (selection.rangeCount > 0) {
                                    ...
                                    if (parentAnchor && parentAnchor.nodeName === "A") {
                                        ...
                                        newAnchorElement.href = updatedUrl;
                                        newAnchorElement.textContent = updatedDescription || updatedUrl;
                                        parentAnchor.parentNode.replaceChild(newAnchorElement, parentAnchor);
                                    } else {
                                        // If no anchor tag is found, insert the anchor element at the cursor position
                                        editor.wysiwygEditorInsertHtml(newAnchor);
                                    }
                                }
                            }
                            setTimeout(() => {
                        ...
                        // If the selected text doesn't contain a link, proceed as usual
                        // Create the dropdown with empty fields
                        var content = document.createElement("div");
                        content.innerHTML = `
                            <label for="url">URL:</label>
                        ...
                        content.querySelector(".button").addEventListener("click", function(e) {
                            ...
                            if (url) {
                                editor.wysiwygEditorInsertHtml(`<a href="${ze(url)}">${ze(description || url)}</a>`);
                            }
        ...
        r.call = function() {
            ...
            if (r.exists(e))
                for (var t = l.length; t--;)
                    if (l[t] instanceof w[e]) return !0;
            return !1
            ...
            var t, n = l.length,
                o = !1;
            if (r.isRegistered(e))
                for (; n--;) l[n] instanceof w[e] && (o = !0, "destroy" in (t = l.splice(n, 1)[0]) && t.destroy.call(i));
            return o
        }, r.destroy = function() {
    ...
    function Tt(r, e, i) {
        var a, l, c = e || r.contentDocument || r.document,
        ...
        g.insertHTML = function(e, t) {
            ...
            var a, l, c, s, u, d, f, p = "(^|[\\s    ])",
                m = e.length,
                h = r ? 1 : 0,
                n = o || e[m - 1][0].length;
            for (r && n++, i = i || "", s = (a = g.getOuterText(!0, n)).length, a += i, t && (a += g.getOuterText(!1, n)); m--;)
                if (f = (d = e[m][0]).length, c = Math.max(0, s - f - h), l = -1, r ? (u = a.substr(c).match(new RegExp(p + wt(d) + p))) && (l = u.index + c + u[1].length) : l = a.indexOf(d, c), -1 < l && l <= s && s <= l + f + h) return g.selectOuterText(u = s - l, f - u - (/^\S/.test(i) ? 1 : 0)), g.insertHTML(e[m][1]), !0;
            return !1
    ...
    var T, C, e, t = navigator.userAgent,
    ...
    var on = function L(e) {
            ...
            if (s.version = "2.4.3", s.removed = [], l && l.document && 9 === l.document.nodeType) {
                            ...
                            if (H("uponSanitizeElement", e, {
                                ...
                                if (ge && !k[i]) {
                                    var a = f(e) || e.parentNode,
                                        l = V(e) || e.childNodes;
                                    if (l && a)
                                        for (var c = l.length - 1; 0 <= c; --c) a.insertBefore(j(l[c], !0), W(e))
                                }
                            }
                        }
                        return O(e), !0
                        ...
                        if (t) {
                            for (var n = {
                                ...
                                var r = (l = t[o]).name,
                                    ...
                                    l = g(r);
                                if (n.attrName = l, n.attrValue = a, n.keepAttr = !0, n.forceKeepAttr = void 0, H("uponSanitizeAttribute", e, n), a = n.attrValue, !n.forceKeepAttr && (_e(r, e), n.keepAttr))
                                    if (Ve(/\/>/i, a)) _e(r, e);
                                    else {
                ...
                s.sanitize = function(e) {
                    var t, n, o, r, i = 1 < arguments.length && void 0 !== arguments[1] ? arguments[1] : {};
                    ...
                    } else if (e instanceof u) 1 === (i = (t = Oe("\x3c!----\x3e")).ownerDocument.importNode(e, !0)).nodeType && "BODY" === i.nodeName || "HTML" === i.nodeName ? t = i : t.appendChild(i);
                    else {
                    ...
                    if (C) {
                        if (E)
                            for (r = $.call(t.ownerDocument); t.firstChild;) r.appendChild(t.firstChild);
                        else r = t;
    ...
    function Ye(r, e) {
        var a, w, u, i, o, f, d, l, c, s, p, t, m, h, g, y, v, L, I, n, b, z, B, x, T, P, U, j, W, V, G, q, $, Y, K, X, Z, J, Q, ee, C, te, ne, oe, re, ie, E, ae, S, le, ce, se, ue, de, fe, N, pe, k, me, D, M = this,
            A = {},
            he = [],
            R = [],
            _ = {},
        ...
        ye.addHook("uponSanitizeElement", function(e, t) {
            ...
            De($e, "click", re), e && (De(e, "reset", C), De(e, "submit", M.updateOriginal, !0)), De(window, "pagehide", M.updateOriginal), De(window, "pageshow", C), De(f, "keypress", ee), De(f, "keydown", J), De(f, "keydown", Q), De(f, "keyup", S), De(f, "blur", pe), De(f, "keyup", k), De(f, "paste", K), De(f, "cut copy", X), De(f, t, ne), De(f, o, le), De(f, n, oe), F.emoticonsCompat && qe.getSelection && De(f, "keyup", de), De(f, "blur", function() {
                M.val() || Fe(f, "placeholder")
            }), De(f, "focus", function() {
                it(f, "placeholder")
            }), De(l, "blur", pe), De(l, "keyup", k), De(l, "keydown", J), De(l, t, ne), De(l, n, oe), De(d, "mousedown", te), De(d, o, le), De(d, "keyup", S), De(w, "selectionchanged", ce), De(w, "selectionchanged", E), De(w, "selectionchanged valuechanged nodechanged pasteraw paste", oe)
        }, V = function() {
            var i, a = M.commands,
            ...
            var e = Ce("div", {
                ...
                b = function(e) {
                    f && (f = !1, et(t), it(w, "resizing"), Me($e, n, v), Me($e, o, b), e.preventDefault())
                };
            T && T.create && ((d = T.create("grip")) && (Ne(e, d), Fe(e, "has-icon"))), Ne(w, e), Ne(w, t), et(t), De(e, "touchstart mousedown", function(e) {
                i = ("touchstart" === e.type ? (e = qe.event, r = e.touches[0].pageX, e.touches[0]) : (r = e.pageX, e)).pageY, c = at(w), s = Le(w), f = !0, Fe(w, "resizing"), tt(t), De($e, n, v), De($e, o, b), e.preventDefault()
            })
        }, Y = function() {
            var e = F.emoticons,
                n = F.emoticonsRoot || "";
            ...
            if (ct(w))
                if (M.sourceMode()) t = e ? l.value.length : 0, l.setSelectionRange(t, t);
                else {
                    if (ht(f), e)
                        for ((n = f.lastChild) || (n = Ce("p", {}, d), Ne(f, n)); n.lastChild;) _e(n = n.lastChild, "br") && n.previousSibling && (n = n.previousSibling);
                ...
                function e(t) {
                    if (t.nodeType === xe) {
                        ...
                        if (Ie(t) && "BR" !== o) {
                            ...
                            if (!t.style.length)
                                if (Qe(t, "style"), "FONT" === o && (Re(t, "fontFamily").toLowerCase() === Re(n, "fontFamily").toLowerCase() && Qe(t, "face"), Re(t, "color") === Re(n, "color") && Qe(t, "color"), Re(t, "fontSize") === Re(n, "fontSize") && Qe(t, "size")), !t.attributes.length && /SPAN|FONT/.test(o)) vt(t);
                                else if (/B|STRONG|EM|SPAN|FONT/.test(o))
                                for (var a = /B|STRONG/.test(o), l = "EM" === o; n && Ie(n) && (!a || /bold|700/i.test(Re(n, "fontWeight"))) && (!l || "italic" === Re(n, "fontStyle"));) {
            ...
            var o = Le(i);
            M.focus(), !n && Ee(y, "code") || (m.insertHTML(e, t), m.saveRange(), P(), mt(f), an(f, d), tt(n = ke(f, "#sceditor-end-marker")[0]), e = f.scrollTop, t = gt(n).top + 1.5 * n.offsetHeight - o, et(n), (e < t || t + o < e) && (f.scrollTop = t), N(!1), m.restoreRange(), S())
            ...
            var n, o = l.selectionStart,
                r = l.selectionEnd,
                i = l.scrollTop;
            l.focus(), n = l.value, t && (e += n.substring(o, r) + t), l.value = n.substring(0, o) + e + n.substring(r, n.length), l.selectionStart = o + e.length - (t ? t.length : 0), l.selectionEnd = l.selectionStart, l.scrollTop = i, l.focus(), N()
            ...
            F.emoticonsEnabled && (e = f, l = O, c = F.emoticonsCompat, s = e.ownerDocument, t = "(^|\\s| | | | |$)", u = [], d = {}, Je(e, "code") || (we(l, function(e) {
                ...
                for (t = t.firstChild; t;) {
                    if (t.nodeType !== xe || _e(t, "code") || e(t), t.nodeType === Te)
                        for (var n = 0; n < u.length; n++) {
                            var o, r = t.nodeValue,
                                i = u[n],
                                a = c ? r.search(d[i]) : r.indexOf(i); - 1 < a && (a = r.indexOf(i, a), o = ut(l[i], s), i = r.substr(a + i.length), o.appendChild(s.createTextNode(i)), t.nodeValue = r.substr(0, a), t.parentNode.insertBefore(o, t.nextSibling))
                        }
                    t = t.nextSibling
                }
            }(e)))
            ...
            if (M.focus(), !Ee(m.parentNode(), "code")) {
                ...
                } catch (e) {}!n && o && o.errorMessage && alert(M._(o.errorMessage)), E()
            }
        }, le = function() {
            function e() {
            ...
            for (var r, i, a = (e = e.split(" ")).length; a--;) Xe(t) && (r = "scewys" + e[a], i = "scesrc" + e[a], n || (A[r] = A[r] || [], A[r].push(t)), o || (A[i] = A[i] || [], A[i].push(t)), "valuechanged" === e[a] && (N.hasHandler = !0));
            return M
            ...
            return e || !1 === e ? ((F.emoticonsEnabled = e) ? (De(f, "keypress", ue), M.sourceMode() || (m.saveRange(), P(), N(!1), m.restoreRange())) : (we(ke(f, "img[data-sceditor-emoticon]"), function(e, t) {
                var n = ot(t, "sceditor-emoticon"),
                    n = d.createTextNode(n);
                t.parentNode.replaceChild(n, t)
            }), Me(f, "keypress", ue), N()), M) : F.emoticonsEnabled
        }, M.css = function(e) {
            return n || (n = Ce("style", {
                id: "inline"
            ...
            var t, n, o;
            h && (h.hasHandler("valuechangedEvent") || N.hasHandler) && (o = !(n = M.sourceMode()) && m.hasSelection(), e = (s = !1) !== e && !d.getElementById("sceditor-start-marker"), p && (clearTimeout(p), p = !1), o && e && m.saveRange(), (t = n ? l.value : f.innerHTML) !== N.lastVal && (N.lastVal = t, lt(w, "valuechanged", {
                rawValue: n ? M.val() : t
            })), o && e && m.removeMarkers())
        }, pe = function() {
            p && N()
            ...
            var e = e.which,
                ...
                n = 13 === t || 32 === t,
                t = 8 === t || 46 === t;
            k.lastChar = e, s || (13 === e || 32 === e ? n ? k.triggerNext = !0 : N() : 8 === e || 46 === e ? t ? k.triggerNext = !0 : N() : k.triggerNext && (N(), k.triggerNext = !1), clearTimeout(p), p = setTimeout(function() {
                s || N()
            }, 1500))
        }, ne = function(e) {
